...
def _post_opencode_go(messages, max_tokens, temperature, timeout):
    r = requests.post(
...
def engines():
    chain = []
    if OPENCODE_GO_KEY:
        chain.append((f"opencode-go:{OPENCODE_GO_MODEL}", _post_opencode_go))
...
def chat_json(messages, max_tokens=6000, temperature=0.7, retries=2):
    ...
    for _ in range(retries + 1):
        out = chat(msgs, max_tokens=max_tokens, temperature=min(0.7, temperature))
        parsed = None
