...
The desktop UI intentionally exposes draft-save only. Settings, Naver session
state and logs are stored in the user's LocalAppData/NaverBlogWriter folder so
replacing the portable EXE does not erase them.
"""
...
# PyInstaller --windowed can set these to None. Legacy modules call reconfigure
# during import, so provide real streams before importing the existing engine.
...
for folder in ("logs", "downloads", "images_pool"):
...
if not CONFIG_PATH.exists():
    src = RESOURCE_DIR / "config.json"
    if not src.exists():
        raise RuntimeError(f"config.json을 찾을 수 없습니다: {src}")
...
class BlogWriterApp:
    def __init__(self, root: tk.Tk):
        ...
        self.root.after(400, self.initial_status)

    def build_ui(self):
        outer = ttk.Frame(self.root, padding=16)
        outer.pack(fill="both", expand=True)
        ttk.Label(outer, text="네이버 블로그 글쓰기", font=("Malgun Gothic", 20, "bold")).pack(anchor="w")
        ttk.Label(
            outer,
            text="주제 입력 → 자료조사 → AI 작성/검수 → 이미지 수집 → 네이버 임시저장",
            font=("Malgun Gothic", 10),
        ).pack(anchor="w", pady=(4, 12))

        tabs = ttk.Notebook(outer)
        tabs.pack(fill="both", expand=True)
        write_tab = ttk.Frame(tabs, padding=14)
        settings_tab = ttk.Frame(tabs, padding=14)
        tabs.add(write_tab, text="글 작성")
        tabs.add(settings_tab, text="설정")

        state_row = ttk.Frame(write_tab)
        state_row.pack(fill="x", pady=(0, 10))
        ttk.Label(state_row, textvariable=self.api_status_var).pack(side="left")
        ttk.Label(state_row, text="   |   ").pack(side="left")
        ttk.Label(state_row, textvariable=self.naver_status_var).pack(side="left")

        ttk.Label(write_tab, text="글 주제", font=("Malgun Gothic", 11, "bold")).pack(anchor="w")
        topic_entry = ttk.Entry(write_tab, textvariable=self.topic_var, font=("Malgun Gothic", 12))
        topic_entry.pack(fill="x", pady=(5, 12), ipady=6)
        topic_entry.bind("<Return>", lambda _e: self.start_draft())
        ...
        actions = ttk.Frame(write_tab)
        actions.pack(fill="x", pady=(0, 10))
        self.write_btn = ttk.Button(actions, text="글 작성 후 네이버 임시저장", command=self.start_draft)
        self.write_btn.pack(side="left", ipadx=14, ipady=7)
        ttk.Button(actions, text="네이버 로그인", command=self.start_login).pack(side="left", padx=8, ipady=7)
        ttk.Button(actions, text="상태 확인", command=self.start_checks).pack(side="left", ipady=7)
        ...
        row = ttk.Frame(settings_tab)
        row.pack(fill="x", pady=(14, 6))
        ttk.Button(row, text="설정 저장", command=self.save_settings).pack(side="left")
        ttk.Button(row, text="API 연결 테스트", command=self.start_api_probe).pack(side="left", padx=8)
        ttk.Button(row, text="데이터 폴더 열기", command=self.open_data_dir).pack(side="left")

        ttk.Separator(settings_tab).pack(fill="x", pady=15)
        ttk.Label(
            settings_tab,
            text=(
                f"설정/로그/로그인 세션 저장 위치\n{DATA_DIR}\n\n"
    ...
    def set_busy(self, value: bool, status: str):
        self.busy = value
        self.write_btn.configure(state="disabled" if value else "normal")
        (self.progress.start if value else self.progress.stop)(12) if value else self.progress.stop()
        self.status_var.set(status)

    def run_background(self, label: str, fn, on_success=None):
        if self.busy:
        ...
        def worker():
            try:
                result = fn()
                self.root.after(0, lambda: self.task_done(True, result, on_success))
            except Exception as exc:
                traceback.print_exc()
                self.root.after(0, lambda e=exc: self.task_done(False, e, None))

        threading.Thread(target=worker, daemon=True).start()

    def task_done(self, ok: bool, result, on_success):
        self.set_busy(False, "완료" if ok else "오류 발생")
    ...
    def save_settings(self, quiet=False):
        blog_id, key, model = self.apply_settings_runtime()
        values = {
            "NAVER_BLOG_ID": blog_id,
            "OPENCODE_GO_API_KEY": key,
            "OPENCODE_GO_MODEL": model,
        ...
        for key_name, value in values.items():
            set_key(str(ENV_PATH), key_name, value, quote_mode="always")
        if not quiet:
            messagebox.showinfo("저장 완료", "설정을 저장했습니다.")

    def initial_status(self):
        self.api_status_var.set("API: 키 설정됨" if self.api_key_var.get().strip() else "API: 키 입력 필요")
        self.naver_status_var.set("네이버: 로그인 세션 있음" if STATE_PATH.exists() else "네이버: 로그인 필요")
    ...
    def start_api_probe(self):
        ...
        self.run_background("API 연결 확인 중...", task, lambda _r: self.api_status_var.set("API: 연결됨"))
    ...
    def start_login(self):
        ...
        self.run_background("브라우저에서 네이버 로그인 중...", task, done)

    def check_naver(self) -> bool:
        if not STATE_PATH.exists():
            return False
        pw = browser = None
        previous = os.environ.get("HEADLESS")
        try:
            os.environ["HEADLESS"] = "1"
            pw, browser, _ctx, page = naver.launch(headless=True)
            return bool(naver.is_logged_in(page))
        finally:
            os.environ["HEADLESS"] = previous if previous is not None else "0"
            try:
                if browser:
                    browser.close()
            except Exception:
    ...
    def start_checks(self):
        ...
        self.run_background("연결 상태 확인 중...", task, done)

    def create_draft(self, topic: str, extra: str):
        self.save_settings(quiet=True)
        if not llm.OPENCODE_GO_KEY:
            raise RuntimeError("설정 탭에서 OpenCode Go API Key를 먼저 입력해 주세요.")
        if not STATE_PATH.exists():
            raise RuntimeError("'네이버 로그인' 버튼을 먼저 눌러 주세요.")
        ...
        cfg = load_config()
        count = min(len(post.get("image_queries", [])), cfg["format"]["image_max"])
        print("이미지 수집 중...")
        imgs = imgmod.fetch_images(post.get("image_queries", []), count)
        print("이미지", len(imgs), "장 확보")

        print("네이버 에디터 입력 및 임시저장 중...")
        result = naver.write_post(post, imgs, mode="draft")
        ok = bool(result.get("ok"))
        record = {
            "ts": datetime.now().astimezone().isoformat(timespec="seconds"),
            "title": post.get("title"), "mode": "draft", "slot_id": "desktop",
            "content_id": cid, "chars": post.get("_chars"), "images": len(imgs),
            "angle": post.get("_angle", {}).get("angle"),
            "keyword": post.get("_angle", {}).get("keyword"),
            "quality": quality,
            "status": "draft" if ok else "failed", "ok": ok,
        }
    ...
    def start_draft(self):
        ...
        def done(post):
            ...
            messagebox.showinfo("임시저장 완료", f"네이버에 임시저장했습니다.\n\n제목: {post.get('title', '')}")

        self.run_background("자료조사와 글 작성을 시작합니다...", lambda: self.create_draft(topic, extra), done)
